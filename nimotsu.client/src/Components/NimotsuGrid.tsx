...
function NimotsuGridLayout() {
    const contentBody = store.nimotsuContainers.map((nimotsuContainer: NimotsuContainer) =>
    ...
    const contentBodyPlaceholder = (<tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700"><td colSpan={4} className="text-center text-lg px-6 py-3">You Luggages are Loading </td></tr>);
    const contents =
        <div className="relative overflow-x-auto">
            <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                    {store.nimotsuContainers.length === 0 ? contentBodyPlaceholder : contentBody}
                </tbody>
            </table>
        </div>;
